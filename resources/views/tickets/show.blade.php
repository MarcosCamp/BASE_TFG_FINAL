        ...
        <div class="space-y-8">
            @foreach($order->items as $item)
                ...
                <div class="bg-white rounded-2xl overflow-hidden shadow-2xl flex flex-col md:flex-row relative">
                    ...
                    {{-- Decoración: Círculos laterales para simular el corte del ticket --}}
                    ...
                    <div class="md:w-1/3 h-48 md:h-auto relative">
                        ...
                        @endif
                        <div class="absolute inset-0 bg-black/30 flex items-center justify-center md:hidden">
                    ...
                    <div class="md:w-1/3 p-6 flex flex-col justify-center border-b md:border-b-0 md:border-r border-dashed border-gray-300 relative">
                        <span class="text-xs font-bold text-indigo-600 uppercase tracking-wider mb-1">Entrada General</span>
                    ...
                    <div class="md:w-1/3 p-6 bg-gray-50 flex flex-col items-center justify-center text-center">
                        <div class="bg-white p-2 rounded-lg shadow-sm mb-3">
                            {{-- Generamos un código QR con datos únicos --}}
                            {{-- Contenido: ID_PEDIDO - ID_ITEM - ID_USUARIO --}}
                            {!! QrCode::size(120)->generate("TICKET-" . $order->id . "-" . $item->id . "-" . Auth::id()) !!}
                        </div>
                        <p class="text-[10px] text-gray-400 font-mono break-all">ID: {{ $order->id }}-{{ $item->id }}-{{ strtoupper(Str::random(5)) }}</p>
                        <span class="inline-block mt-2 px-3 py-1 bg-green-100 text-green-800 text-xs font-bold rounded-full">
                            {{ $item->quantity }} {{ Str::plural('Persona', $item->quantity) }}
                        </span>
                    </div>
                </div>
            @endforeach
